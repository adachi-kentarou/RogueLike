...
namespace Map.Model
{
    ...
    partial class MapModel
	{
		...
		private bool CreateRoadStairsStraightBottom(CellData a_data, Point a_point, GameObject a_parent)
		{
			//直線階段判定
			...
			//上の直線階段判定
			var t_data = Map.Param.CommonParams.GetCellData(a_point + Point.up);
			...
			//上下の接続方向チェック
			var t_ret = false;
			switch (t_connect_direction_up)
			{
				case Map.Direction.RIGHT:
					if (t_connect_direction_down == Map.Direction.LEFT) t_ret = true;
					break;
				case Map.Direction.LEFT:
					if (t_connect_direction_down == Map.Direction.RIGHT) t_ret = true;
					break;
				case Map.Direction.FRONT:
					if (t_connect_direction_down == Map.Direction.BACK) t_ret = true;
					break;
				case Map.Direction.BACK:
					if (t_connect_direction_down == Map.Direction.FRONT) t_ret = true;
					break;
			}

			if (t_ret == false) return false;

			var t_obj = CreateCellObject(ObjeType.ROAD_STAIRS_STRAIGHT_BOTTOM, t_connect_direction_down, a_data, a_point, a_parent);
