...
namespace Map.Model
{
    ...
    partial class MapModel
	{
		...
		private bool CreateRoomDivisionWall(CellData a_data, Point a_point, GameObject a_parent)
		{
			...
			for (int ii = (int)Map.Direction.LEFT; ii < (int)Map.Direction.MAX_NUM; ii++)
			{
				...
				if (a_data.GetConnect(t_direction) != Map.Cell.ConnectType.CONNECT)
				{
					...
					if (Map.Param.CommonParams.m_map_area.IsAreaIn(t_next_cell_point) == true)
					{
						...
						if (t_next_cell_data.m_room_area == a_data.m_room_area && a_data.GetConnect(t_direction) != ConnectType.CONNECT)
						{
							t_ret = true;
							var t_obj = CreateCellObject(ObjeType.ROOM_DIVISION_WALL, t_direction, a_data, a_point, a_parent);
						}
					}
				}
			}
